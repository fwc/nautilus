...
void MLIRLoweringProvider::generateMLIR(ir::CastOperation* castOperation, MLIRLoweringProvider::ValueFrame& frame) {
	...
	if (isFloat(inputStamp) && isFloat(outputStamp)) {
		...
		auto mlirCmp = builder->create<mlir::arith::CmpIOp>(getNameLoc("location"),
		  mlir::arith::CmpIPredicate::ne, mlirInput, mlirZero);
		frame.setValue(castOperation->getIdentifier(), mlirCmp);
		return;
	}

	throw NotImplementedException(fmt::format("Cast from {} to {} is not supported.", toString(inputStamp), toString(outputStamp)));
}
