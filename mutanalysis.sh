...
find build/nautilus/test/yarpgened -type f -name "*_test_69*" -print0 | xargs -0 --max-procs=$(nproc) -I {} sh -c "./{} > /dev/null 2> /dev/null && echo {} >> $working_tests || echo {} >> $broken_tests"
...
do
    ...
    cat $working_tests | xargs --max-procs=$(nproc) -I {} timeout 1m sh -c "./{} > /dev/null 2> /dev/null || echo mutant $patch killed by $(basename {})" | tee -a $out_log || true
    mv $patched_file.bak $patched_file
done
